  ...
  dynamic "data_profile_spec" {
    ...
    content {
      ...
      dynamic "post_scan_actions" {
        for_each = var.data_profile_spec.post_scan_actions != null ? [""] : []
        content {
          dynamic "bigquery_export" {
            for_each = (
              var.data_profile_spec.post_scan_actions.bigquery_export != null
              ? [""]
              : []
            )
            content {
              results_table = try(
                var.data_profile_spec.post_scan_actions.bigquery_export.results_table,
                null
              )
            }
          }
